...
public class BehaviorRush {
	...
	public BehaviorRush(BasicExpandedConfigurableScript bec, PhysicalGameState pgs, Player p, GameState gs, int player)
	{
		this.bec=bec;
		this.pgs=pgs;
		this.p=p;
	...
	public void baseBehavior(Unit u, Player p, PhysicalGameState pgs) {
		if (unitTypeL.contains(bec.parametersValues.getWorker())
				&& p.getResources() >= bec.workerType.cost + bec.resourcesUsed) {
			bec.train(u, bec.workerType);
			bec.resourcesUsed+=bec.workerType.cost;
		}
		else if(!(unitTypeL.contains(bec.parametersValues.getWorker()))
				&& p.getResources() >= bec.workerType.cost + bec.resourcesUsed && bec.nworkers < 1)
		{
			bec.train(u, bec.workerType);
			bec.resourcesUsed+=bec.workerType.cost;
		}
...
//		while(bec.choices.get(BasicExpandedChoicePoint.UNITTYPE)==bec.parametersValues.getWorker() &&
//				freeWorkers.size()>1)
