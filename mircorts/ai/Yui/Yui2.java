...
public class Yui2 extends AbstractionLayerAI {
	UnitTypeTable nutt;
	UnitType workerType;
	UnitType baseType;
	...
	public Yui2(UnitTypeTable utt) {

		this(utt, new AStarPathFinding());

	}

	public Yui2(UnitTypeTable utt, AStarPathFinding aStarPathFinding) {
...
//
		...
		for (Unit u : pgs.getUnits()) {
			...
			/*
			 * if (u.getType().canAttack &&!u.getType().canHarvest&& u.getPlayer() ==
			 * player) { troop.add(u); }
			 */
			if (u.getType() == baseType && u.getPlayer() == player) {
				base = u;
				if (base.getX() < (0+pgs.getWidth() / 2)) {
					top = true;
				}
			}
//			if (u.getPlayer() == -1) {
			...
			while (troop.size() > 0) {
				freeTroop.add(troop.remove(0));
					...
					if(maxHarvest>0) {
				       if (freeTroop.size() >= 2) {
						Troopattack = true;
						for (Unit u : freeTroop)
							attackUnitBehavior(u, p, pgs);
						for (Unit u : freeTroop)
							freeTroop.removeAll(freeTroop);
					   }
					...
					/*
					 * Unit target = base; for(int i=0;i<freeTroop.size();i++) { Unit unit =
					 * freeTroop.get(i); if(!top) { move(unit, target.getX()-1-i,target.getY());
					 * System.out.println(target.getX()); }else { move(unit,
					 * target.getX()+1+i,target.getY()); } }
					 */
				    }
...
//			}
//		}
		if (p.getResources() >= barracksType.cost && freeWorkers.size() >= 1 && nbarracks <1) {
			Unit u1 = freeWorkers.remove(freeWorkers.size() - 1 );
			int x = base.getX();
			int y = base.getY();
			...
			buildIfNotAlreadyBuilding(u1, barracksType, x, y, reservedPositions, p, pgs);
			nbarracks++;
		}
